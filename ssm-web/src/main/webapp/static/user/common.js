    ...
    form.validate({
        ...
        WMHandler: function () {
            if ($(".layui-layer-tips").length > 0)
                Metronic.scrollTo($(".layui-layer-tips"), 0, window.top);
        },
...
//select联动如果只有一个值的时候不显示请选择
function select_link1(params, tt, typeId, queryParams1, queryParams2) {
    var arr = params.split(',');
    var selecter = [];
    for (var i = 0; i < arr.length; i++) {
        selecter.push(arr[i]);
    }
    var selectValue = $("#" + queryParams1).val();
    var selectValue2 = $("#" + queryParams2).val();
    $.ajax({
        //url: "/pim/myBaseCon/initSelectLink",
        url: rootPath + "/projectApprovalCon/initSelectLinkByAllValue",
        type: "post",//可选get
        dataType: "json",
        data: {
            typeId: typeId,
...
function fileUploadSe(id, dufData) {
    ...
    var fiveData = {
        ...
        onUploadComplete: function (file, data) {
            dufData.successFunction("uploadifive-" + id, file, data);
        }
    };
